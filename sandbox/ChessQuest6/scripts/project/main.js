...
async function OnBeforeProjectStart(runtime)
{
	...
	document.oCanvas.addEventListener("touchend", function(ev) { fEvent_TouchEnd(ev, document.oCanvas); });	
...
function fEvent_TouchEnd(eve, can) {
	let mouseEvent = new MouseEvent("click", {touching: true});
...
function fSetMarker(eve) {
	...
	switch (document.oRuntime.layout.name) {
		case "Battle Board":
			...
			if (!b) {
				if (markerS.isVisible && !eve.touching) {
