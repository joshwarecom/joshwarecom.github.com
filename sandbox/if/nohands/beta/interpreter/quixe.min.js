...
var QuixeClass = function() {
    ...
    function quixe_getlibrary(val) {
        switch (val) {
            ...
            case 'Glk':
                return self.Glk;
            case 'GlkOte':
                return self.Glk.getlibrary('GlkOte');
            case 'Dialog':
                return self.Glk.getlibrary('Dialog');
        }
        return null;
    }
    ...
    function qobjdump(obj, depth) {
        ...
        if (jQuery.type(obj) === 'array') {
            if (depth)
                depth--;
            var ls = jQuery.map(obj, function(v, ix) {
                return qobjdump(v, depth);
            });
            return ("[" + ls.join(",") + "]");
        }
    ...
    function VMFunc(funcaddr, startpc, localsformat, rawformat) {
        ...
        this[2] = {};
        this.locallen = null;
        this.localsformat = localsformat;
    ...
    var opcode_table = {
        ...
        0x4b: function(context, operands) {
            if (quot_isconstant(operands[1])) {
                ...
                bitnum = Number(operands[1]) & 0xffffffff;
                bitx = bitnum & 7;
                if (quot_isconstant(operands[0])) {
        ...
        0x110: function(context, operands) {
            var expr;
            if (quot_isconstant(operands[0])) {
                var val = Number(operands[0]) & 0xffffffff;
                if (val == 0)
    ...
    function pop_callstub(val) {
        ...
        if (framestart != self.frame.framestart) {
            valstack.length -= 1;
            fatal_error("Call stub frameptr (" + framestart + ") " + "does not match frame (" + self.frame.framestart + ")");
        }
    ...
    function set_string_table(addr) {
        ...
        if (textenv === undefined) {
            ...
            if (cache_stringtable) {
                ...
                build_decoding_tree(tmparray, rootaddr, 4, 0);
                dectab = tmparray[0];
                if (dectab === undefined)
    ...
    function decode_float(val) {
        ...
        expo = (val >> 23 & 0xff);
        if (expo) {
            res = ((val & 0x7fffff | 0x800000) / 8388608 * Math.pow(2, (expo - 127)));
        } else {
            res = ((val & 0x7fffff) / 8388608 * Math.pow(2, -126));
        }
        if (sign)
            return -res;
        else
    ...
    self.vm_stopped = false;
    self.tempcallargs = null;
    self.tempglkargs = null;
    self.done_executing = null;
    var vmfunc_table;
    var vmtextenv_table;
    var decoding_tree;
    var vmstring_table;
    self.random_func = null;
    var ramstart;
    var endgamefile;
    var origendmem;
    var stacksize;
    ...
    function unpack_iff_chunks(bytes) {
        ...
        while (pos < bytes.length) {
            ...
            if ((pos + size) > bytes.length) {
                qlog(key + " chunk is truncated " + "(" + size + " bytes needed, " + (bytes.length - pos) + " available");
                return undefined;
            }
    ...
    function vm_restore(streamid) {
        ...
        if (heapchunk && heapchunk.length >= 8) {
            ...
            usedlist.sort(function(blk1, blk2) {
                return blk1.addr - blk2.addr;
            });
    ...
    self.VMStopped = {
        dummy: 'The top-level function has returned.'
    };
    ...
    function execute_loop() {
        ...
        pathend = new Date().getTime();
        total_execution_time += (pathend - pathstart) / 1000.0;
        if (self.vm_stopped) {
            self.Glk.glk_exit();
        }
        self.Glk.update();
...
var GiDispaClass = function() {
    ...
    function build_function(func) {
        ...
        for (ix = 0; ix < form.args.length; ix++) {
            ...
            argjoin.push(tmpvar);
            locals[tmpvar] = true;
            if ((arg instanceof ArgInt) || (arg instanceof ArgChar) || (arg instanceof ArgClass)) {
                ...
                out.push('} else {');
                if ((refarg instanceof ArgInt) || (refarg instanceof ArgChar) || (refarg instanceof ArgClass)) {
    ...
    function check_autosave() {
        if (blocked_selector == 0x0C0 && blocked_callargs && blocked_callargs.length > 0) {
            if (last_event_type == 2 || last_event_type == 3 || last_event_type == 4 || last_event_type == 8) {
...
try {
    exports.GiDispaClass = GiDispaClass;
} catch (ex) {};
'use strict';
var GiLoadClass = function() {
    ...
    function load_run(optobj, image, imageoptions) {
        ...
        if (optobj && window.Quixe && ((!optobj.vm) || optobj.vm === window.Quixe)) {
    ...
    function start_game(image) {
        ...
        if (image[0] == 0x46 && image[1] == 0x4F && image[2] == 0x52 && image[3] == 0x4D) {
            ...
            if (all_options.blorb_gamechunk_type) {
                ...
                try {
                    ...
                    all_options.Blorb.init(image, {
                        format: 'blorbbytes',
                        retainuses: retainuses
                    });
        ...
        if (all_options.Blorb && !all_options.Blorb.inited()) {
            if (all_options.image_info_map) {
                all_options.Blorb.init(all_options.image_info_map, {
                    format: 'infomap'
                });
    ...
    function get_library(val) {
        switch (val) {
            case 'GlkOte':
                return GlkOte;
            case 'GiDispa':
                return all_options.GiDispa;
            case 'Blorb':
                return all_options.Blorb;
            case 'VM':
                return all_options.vm;
            case 'IO':
                return all_options.io;
        }
        return null;
    }
    return {
...
try {
    exports.GiLoad = GiLoad;
    exports.GiLoadClass = GiLoadClass;
} catch (ex) {};