...
var GlkOteClass = function() {
    ...
    function glkote_init(iface) {
        ...
        if (Dialog && !Dialog.inited()) {
            ...
            if (Dialog.init_async) {
                Dialog.init_async(dialogiface, function() {
                    finish_init(iface);
                })
                return;
            } else if (Dialog.init) {
    ...
    function measure_window() {
        ...
        var layout_test_pane = $('<div>', {
            id: dom_prefix + 'layout_test_pane'
        });
    ...
    function glkote_update(arg) {
            ...
            if (win.type == 'buffer' && win.needscroll) {
                ...
                if (!win.needspaging) {
                    ...
                    if (!win.needspaging) {
                        ...
                        if (!moreel.length) {
                            moreel = $('<div>', {
                                id: dom_prefix + 'win' + win.id + '_moreprompt',
                                'class': 'MorePrompt'
                            });
                            moreel.append('More');
                        ...
                        if (!prevel.length) {
                            prevel = $('<div>', {
                                id: dom_prefix + 'win' + win.id + '_prevmark',
                                'class': 'PreviousMark'
                            });
        ...
        if (autorestore) {
            ...
            if (!(autorestore.metrics && autorestore.metrics.width == current_metrics.width && autorestore.metrics.height == current_metrics.height)) {
    ...
    function accept_one_window(arg) {
        ...
        if (win.type == 'grid') {
            ...
            if (arg.gridheight > win.gridheight) {
                for (ix = win.gridheight; ix < arg.gridheight; ix++) {
                    var el = $('<div>', {
                        id: dom_prefix + 'win' + win.id + '_ln' + ix,
                        'class': 'GridLine'
                    });
                    el.append(NBSP);
        ...
        if (win.type == 'graphics') {
            ...
            if (!el.length) {
                ...
                if (ctx) {
                    win.backpixelratio = ctx.webkitBackingStorePixelRatio || ctx.mozBackingStorePixelRatio || ctx.msBackingStorePixelRatio || ctx.oBackingStorePixelRatio || ctx.backingStorePixelRatio || 1;
                }
    ...
    function accept_one_content(arg) {
        ...
        if (win.type == 'buffer') {
            ...
            if (divel) {
                var cursel = $('<span>', {
                    id: dom_prefix + 'win' + win.id + '_cursor',
                    'class': 'InvisibleCursor'
                });
                divel.append(cursel);
                if (win.inputel) {
    ...
    function send_response(type, win, val, val2) {
        if (disabled && type != 'specialresponse')
            return;
        if (generation <= generation_sent && !(type == 'init' || type == 'refresh')) {
        ...
        if (!(type == 'init' || type == 'refresh' || type == 'specialresponse' || type == 'debuginput')) {
            jQuery.each(windowdic, function(tmpid, win) {
                var savepartial = (type != 'line' && type != 'char') || (win.id != winid);
                if (savepartial && win.input && win.input.type == 'line' && win.inputel && win.inputel.val()) {
...
var DialogClass = function() {
    ...
    function dialog_open(tosave, usage, gameid, callback) {
        ...
        var form, el, row;
        form = $('<form>');
        form.on('submit', (will_save ? evhan_accept_save_button : evhan_accept_load_button));
        dia.append(form);
        row = $('<div>', {
        ...
        row = $('<div>', {
            ...
            el.on('click', (will_save ? evhan_accept_save_button : evhan_accept_load_button));
            row.append(el);
        }
        form.append(row);
        frame.append(dia);
    ...
    function evhan_storage_changed(ev) {
        ...
        if (editing && editing_dirent) {
            ...
            if (usage_is_textual(editing_dirent.usage)) {
                ...
                var linkel = $('<a>', {
                    'href': 'data:application/octet-stream;base64,' + b64dat,
                    'target': '_blank',
                    'download': 'data'
                });
...
var GlkClass = function() {
                ...
                case Const.wintype_TextGrid:
                    ...
                    for (ix = 0; ix < win.gridheight; ix++) {
                        ...
                        for (cx = 0; cx < win.gridwidth;) {
                            ...
                            for (; cx < win.gridwidth && lineobj.styles[cx] == laststyle && lineobj.hyperlinks[cx] == lasthyperlink; cx++) {}
                            if (lastpos < cx) {
                                if (!lasthyperlink) {
                ...
                case Const.wintype_Graphics:
                    ...
                    if (obj.draw && obj.draw.length) {
                        for (ix = 0; ix < obj.draw.length; ix++) {
                            ...
                            if (drawel.special == 'fill' && drawel.x === undefined && drawel.y === undefined && drawel.width === undefined && drawel.height === undefined) {
    ...
    function get_library(val) {
        switch (val) {
            ...
            case 'Blorb':
                return Blorb;
            case 'Dialog':
                return GlkOte.getlibrary('Dialog');
        }
        return null;
    }
    ...
    function save_allstate() {
        ...
        for (var fref = gli_filereflist; fref; fref = fref.next) {
            var obj = {
                type: fref.type,
                rock: fref.rock,
                disprock: fref.disprock,
                filename: fref.filename,
                textmode: fref.textmode,
                filetype: fref.filetype,
                filetypename: fref.filetypename
            };
            obj.ref = fref.ref;
    ...
    function gli_window_put_string(win, val) {
        ...
        switch (win.type) {
            case Const.wintype_TextBuffer:
                if (win.style != win.accumstyle || win.hyperlink != win.accumhyperlink)
                    gli_window_buffer_deaccumulate(win);
                win.accum.push(val);
                break;
            case Const.wintype_TextGrid:
            ...
            case strtype_File:
                if (str.streaming) {
                    if (!str.unicode) {
                        ...
                        if (!str.isbinary) {
                            var buf = str.fstream.BufferClass.from(val);
                            str.fstream.fwrite(buf);
                        } else {
    ...
    function glk_window_open(splitwin, method, size, wintype, rock) {
        ...
        if (!gli_rootwin) {
            ...
            if (val != Const.winmethod_Above && val != Const.winmethod_Below && val != Const.winmethod_Left && val != Const.winmethod_Right)
    ...
    function glk_window_set_arrangement(win, method, size, keywin) {
        ...
        if (keywin && keywin.type == Const.wintype_Blank && (method & Const.winmethod_DivisionMask) == Const.winmethod_Fixed)
    ...
    function glk_stream_open_file(fref, fmode, rock) {
        ...
        var str;
        var fstream;
        if (fmode != Const.filemode_Read && fmode != Const.filemode_Write && fmode != Const.filemode_ReadWrite && fmode != Const.filemode_WriteAppend)
        ...
        str = gli_new_stream(strtype_File, (fmode != Const.filemode_Write), (fmode != Const.filemode_Read), rock);
        str.unicode = false;
    ...
    function glk_stream_open_memory(buf, fmode, rock) {
        var str;
        if (fmode != Const.filemode_Read && fmode != Const.filemode_Write && fmode != Const.filemode_ReadWrite)
            throw ('glk_stream_open_memory: illegal filemode');
        str = gli_new_stream(strtype_Memory, (fmode != Const.filemode_Write), (fmode != Const.filemode_Read), rock);
        str.unicode = false;
        if (buf) {
            str.buf = buf;
    ...
    function glk_request_line_event(win, buf, initlen) {
        ...
        if (win.type == Const.wintype_TextBuffer || win.type == Const.wintype_TextGrid) {
            if (initlen) {
    ...
    function glk_request_mouse_event(win) {
        ...
        if (win.type == Const.wintype_TextGrid || win.type == Const.wintype_Graphics) {
